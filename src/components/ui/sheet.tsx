...
const sheetVariants = cva(
  "fixed z-50 gap-4 bg-surface shadow-2xl transition ease-in-out data-[state=closed]:duration-300 data-[state=open]:duration-300 data-[state=open]:animate-in data-[state=closed]:animate-out",
  {
    variants: {
      side: {
...
interface SheetContentProps
  ...
  /** Hide the close button (useful when the sheet has its own header). */
  hideClose?: boolean;
}
...
>(
  (
    { side = "left", className, children, hideClose = false, ...props },
    ref,
  ) => (
    <SheetPortal>
      <SheetOverlay />
